...
enum GroqAI {
    ...
    КРИТИЧЕСКОЕ ПРАВИЛО — ОБСУЖДЕНИЕ ПЕРЕД ДЕЙСТВИЕМ:
    1. Когда пользователь просит добавить тренировку или привычку, НЕ добавляй её сразу.
    2. Сначала задай вопросы: какие цели? какой уровень подготовки? сколько времени готов тратить? какие предпочтения?
    3. Предложи варианты, обсуди план вместе с пользователем.
    4. Только когда пользователь ЯВНО согласится (скажет "да", "давай", "ок", "добавляй"), используй команды ADD_TASK или ADD_HABIT.
    5. Если пользователь ещё не согласился — продолжай обсуждение, НЕ отправляй команды добавления.
    6. Это правило абсолютное и не имеет исключений.
